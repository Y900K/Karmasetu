...
import { normalizeQuizQuestions } from '@/lib/courseUtils';
...
export async function POST(request: Request) {
  try {
    const admin = await requireSecureAdminMutation(request, 'admin_generate_quiz');
    if (!admin.ok) {
    ...
    const generatedQuestions = await generateAdminCourseQuiz(topic, apiKey, count, languageMode);
    const questions = normalizeQuizQuestions(generatedQuestions, count);

    if (questions.length === 0) {
      await logSystemEvent(
        'ERROR',
        'admin_generate_quiz',
        'Quiz generation returned no valid questions after normalization.',
        { actorAdminId: session.user._id.toString(), topic, count, languageMode },
        session.user._id.toString()
      );

      return NextResponse.json(
        { ok: false, message: 'Failed to generate valid quiz questions. Please retry.' },
        { status: 502 }
      );
    }
    ...
    await logSystemEvent(
      ...
      { actorAdminId: session.user._id.toString(), topic, countRequested: count, countReturned: questions.length, languageMode },
      session.user._id.toString()
    );
